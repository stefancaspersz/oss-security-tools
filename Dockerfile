...
RUN apt-get update && apt-get install -y git make python3-full vim wget && apt-get clean \
&& wget -q https://go.dev/dl/go1.23.3.linux-arm64.tar.gz \
&& rm -rf /usr/local/go && tar -C /usr/local -xzf go1.23.3.linux-arm64.tar.gz && rm go1.23.3.linux-arm64.tar.gz \
...
&& dpkg -i trivy_0.57.0_Linux-ARM64.deb && rm trivy_0.57.0_Linux-ARM64.deb
